    ...
    public String checkGameWinner(char[][] grid) {
        ...
        for (int i = 0; i < grid.length; i++) {
            if (grid[i][0] != '-' && grid[i][0] == grid[i][1] && grid[i][1] == grid[i][2]) {
                result = Character.toString(grid[i][0]) + " wins ";
                isTie = false;
                return result;
            }
        }
        ...
        for (int j = 0; j < grid[0].length; j++) {
            if (grid[0][j] != '-' && grid[0][j] == grid[1][j] && grid[1][j] == grid[2][j]) {
                result = Character.toString(grid[0][j]) + " wins ";
                isTie = false;
                return result;
            }
        }
        ...
        if (grid[0][0] != '-' && grid[0][0] == grid[1][1] && grid[1][1] == grid[2][2]) {
            result = Character.toString(grid[0][0]) + " wins ";
            isTie = false;
            return result;
        }

        if (grid[0][2] != '-' && grid[0][2] == grid[1][1] && grid[1][1] == grid[2][0]) {
            result = Character.toString(grid[0][2]) + " wins ";
            isTie = false;
            return result;
        }
        for (int i = 0; i < grid.length; i++) {
